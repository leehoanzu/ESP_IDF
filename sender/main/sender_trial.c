...
// Flag trạng thái chặn các task khác chạm vào UART khi đang reset độc lập
static bool is_resetting1 = false; 
static bool is_resetting2 = false; 

#define RADAR_TIMEOUT_MS 15000  // 15 giây không có data = Treo
...
// ================= HÀM CẤP 1: NẠP THAM SỐ CHUNG =================
static void apply_radar_params(uart_port_t port, uint8_t radar_id) {
    send_at_production(port, "AT+STOP\n", "OK", 2000);
    drain_single_uart(port, 100);
    ...
    for(int i=0; i < config_count; i++) {
        send_at_production(port, COMMON_CONFIG[i], "OK", 2000);
    }

    send_at_production(port, "AT+TIME=101\n",   "OK", 2000);

    if (radar_id == RADAR_ID_RIGHT) {
        send_at_production(port, "AT+XNegaD=30\n",  "OK", 2000);
        send_at_production(port, "AT+XPosiD=200\n", "OK", 2000);
    } else {
        send_at_production(port, "AT+XNegaD=-200\n","OK", 2000);
        send_at_production(port, "AT+XPosiD=-30\n", "OK", 2000);
    }
}

// ================= HÀM CẤP 2A: CẤU HÌNH LÚC KHỞI ĐỘNG =================
static void radar_at_config(void) {
    ESP_LOGI(TAG, "=== PHASE 0: HARD RESET BOTH RADARS ===");
    hard_reset_radar(RADAR1_RESET_GPIO);
    hard_reset_radar(RADAR2_RESET_GPIO);

    ESP_LOGI(TAG, "=== PHASE 1 & 2: STOP, DRAIN & TIGHT BOUNDARY CONFIG ===");
    apply_radar_params(RADAR1_UART, RADAR_ID_RIGHT);
    apply_radar_params(RADAR2_UART, RADAR_ID_LEFT);
...
// ================= HÀM CẤP 2B: CẤU HÌNH PHỤC HỒI ĐỘC LẬP =================
static void radar_recovery_config(uart_port_t port, uint8_t radar_id) {
    ESP_LOGW(TAG, "=== WATCHDOG RECOVERY: REBOOTING %s RADAR ===", radar_id_label(radar_id));
    
    int gpio = (radar_id == RADAR_ID_RIGHT) ? RADAR1_RESET_GPIO : RADAR2_RESET_GPIO;
    hard_reset_radar(gpio);

    ESP_LOGI(TAG, "--- REAPPLYING PARAMS FOR %s ---", radar_id_label(radar_id));
    apply_radar_params(port, radar_id);

    ESP_LOGI(TAG, "--- RETRAINING %s RADAR ---", radar_id_label(radar_id));
    uart_write_bytes(port, "AT+START\n", 9);
    drain_single_uart(port, 300); 
    uart_write_bytes(port, "AT+STUDY\n", 9);
    drain_single_uart(port, 4000); 
    send_at_production(port, "AT+STOP\n", "OK", 2000);
    
    ESP_LOGI(TAG, "=== %s RADAR RECOVERY COMPLETE ===", radar_id_label(radar_id));
}
...
static void radar_tdm_task(void *pv) {
    ...
    while (1) {
        ...
        if (!is_resetting1) {
            ...
            vTaskDelay(pdMS_TO_TICKS(600));           // RIGHT hoạt động 600ms
            ...
            vTaskDelay(pdMS_TO_TICKS(100)); // Nhường CPU nếu đang reset
        }
        ...
        if (!is_resetting2) {
            ...
            vTaskDelay(pdMS_TO_TICKS(600));           // LEFT hoạt động 600ms
