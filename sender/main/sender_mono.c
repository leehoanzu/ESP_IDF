...
// (espnow_tx_task, queue_espnow_send, crc16, sendRadarFrame, hard_reset_radar, drain_uart, 
//  active_delay_and_drain, drain_single_uart, send_at_production, radar_id_label... giữ nguyên)
...
static void set_scene_params(uart_port_t port) {
    ...
    for (int i = 0; i < sizeof(PARAMS)/sizeof(PARAMS[0]); i++) {
        send_at_production(port, PARAMS[i], "OK", 2000);
    }
}
...
static void radar_tdm_task(void *pv) {
    ...
    while (1) {
        if (!is_resetting1) {
            ...
            // send_at_production(RADAR1_UART, "AT+RESET\n", "OK", 300);
            // send_at_production(RADAR1_UART, "AT+RESET\n", "OK", 200);
            // uart_write_bytes(RADAR1_UART, "AT+RESET\n", 9);
            // drain_uart(RADAR1_UART);
